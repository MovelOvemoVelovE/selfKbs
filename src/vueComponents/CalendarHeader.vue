<template>
  <div class="jbs_calendar_header">
    <!-- 日历头部 -->
    <slot name="header" v-if="showHeaderTitle">
      <div class="jbs_calendar_btn_group">
        <div class="jbs_calendar_prev" @click="prevMonth" @touch="prevMonth">
          上个月
        </div>
        <slot name="headerTitle">
          <div class="jbs_calendar_title">{{ currentMonth }}</div>
        </slot>
        <div class="jbs_calendar_next" @click="nextMonth" @touch="nextMonth">
          下个月
        </div>
      </div>
    </slot>
    <div class="jbs_calendar_weekdays">
      <span v-for="day in props.weekdays" :key="day">{{ day }}</span>
    </div>
  </div>
</template>
