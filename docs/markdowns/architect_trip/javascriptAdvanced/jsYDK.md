...
以上`bar`函数词法作用域可以访问foo内部的作用域， 而后我们`return bar`
...
调用位置就是函数在代码中被调用(**不是被声明**)的位置。只有仔细分析后，才能回答this到底引用的是什么?
...
- 设置常量： `writable`和`configurable`都设置为false
