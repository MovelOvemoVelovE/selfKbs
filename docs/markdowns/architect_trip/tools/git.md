...
| 操作                          |                 命令                 |
| :---------------------------- | :----------------------------------: |
| **储藏**当前进度              |     `git stash <stash-message>`      |
| **储藏**通过`add .`暂存的文件 | `git stash --staged <stash-message>` |
| **查看储藏**列表              |           `git stash list`           |
| **应用储藏的**进度            |     `git stash apply <stash-id>`     |
| **丢弃储藏的**进度            |     `git stash drop <stash-id>`      |
...
1. `master` 分支写入代码并提交、push 后，需要回退、重置代码
