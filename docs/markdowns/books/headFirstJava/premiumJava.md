...
只要有活着的引用，对象也会存活。 如果最后一个对对象的唯一引用死了，那么对象也会从堆中删除。 实例变量、引用变量也会跟着在堆栈中一起解散。
...
1. **实例变量**
    - 对象的状态就是保持在实例变量的值
    - 局部变量的寿命太短了，可以称为临时变量、栈变量、变量作用域、局部变量
    - 

1. **局部变量**
    - 局部变量同样重要， 对象的行为是方法，方法运行则必须借助局部变量
    - 一个方法调用另一个方法，可能会让我长寿
    - 等待另一个方法调用时，什么也不做，值也不会丢失， 只有范围回归后才会继续活动
